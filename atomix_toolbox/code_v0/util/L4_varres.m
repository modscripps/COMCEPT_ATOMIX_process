function [] = L4_varres(filePI,fileTEST,fileL3TEST,pi,tester)
...
% read in epsi
try
    epsiPI = ncread(filePI,'/L4_dissipation/VAR_RESOLVED');
    novar=0;
catch
    novar=1;
end
if novar==0
    epsiTEST = ncread(fileTEST,'/L4_dissipation/VAR_RESOLVED');
    epsiL3TEST = ncread(fileL3TEST,'/L4_dissipation/VAR_RESOLVED');
    ...
    for ii=1:n
        ...
        try
            plot(timeL3TEST,epsiL3TEST(:,ii),'.-','Color','m','LineWidth',1.5,...
                'MarkerSize',10)
        catch
            plot(timeL3TEST,epsiL3TEST(ii,:),'.-','Color','m','LineWidth',1.5,...
                'MarkerSize',10)
        ...
        if(ii==1)
            title('Variance Resolved','FontSize',15,'FontName','times new roman')
        end
        hold off
        xlim(timePI([1 end]))
        box on
        grid on
        ylabel([char(949) ' shear ' int2str(ii) ' (VAR_RESOLVED\)'])
        if ii==n
            xlabel('time elapsed (seconds)')
            legend(pi,tester,[tester 'L3'] ,'Location','best')
        end
    end
end
