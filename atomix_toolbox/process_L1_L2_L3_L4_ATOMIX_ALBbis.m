function [FieldData]=process_L1_L2_L3_L4_ATOMIX_ALBbis(Meta,GroupMeta,FieldData)
...
if isfield(FieldData.(Levelnames{1}),'PRES_SLOW')
    FieldData.(Levelnames{1}).PRES=FieldData.(Levelnames{1}).PRES_SLOW;
...
sm_PSPD_REL=abs(smoothdata(PSPD_REL,'movmean',16));
% diff FLOWPS
...
for i=1:length(Profile.idx_epsi)
    SECTION_NUMBER(Profile.idx_epsi{i})=i;
end
...
clean_timeseries  = @(x)filloutliers(x,'linear','movmean',mov_mean_window);
...
for tt=1:length(FieldData.PI_TIME)
    ...
    idx_shear  = idx_shear_center+(-disslength/2:disslength/2-1);
%     fprintf("disslength=%f sec\r\n",disslength./Meta.fs_fast)

    if(mean(FieldData.(Levelnames{2}).PSPD_REL(idx_ctd))>speed_limit) && ...
