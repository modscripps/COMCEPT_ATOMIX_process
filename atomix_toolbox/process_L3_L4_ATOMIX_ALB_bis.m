function [FieldData]=process_L3_L4_ATOMIX_ALB_bis(Meta,GroupMeta,FieldData)
...
for d=1:ndim
    wh_dim=GroupMeta.(Levelnames{1}).dimensions{d};
    for v=1:nvar
        wh_var=varnames{v};
        idx_dim=cellfun(@(x) strcmp(x,(wh_dim)),GroupMeta.(Levelnames{1}).variables.(wh_var));
...
for l=1:length(Levelnames)
    ...
    for v=1:nvar
        ...
        if ~isfield(FieldData.(Levelnames{l}),wh_var)
            ...
            for d=1:ndim
                wh_dim=GroupMeta.(Levelnames{l}).variables.(wh_var){d};
                try
...
for p=1:n_disslength
        ...
        SH_SPEC_CLEAN1=Ps1_clean_f .* w ./ TFoakey;
        SH_SPEC_CLEAN2=Ps2_clean_f .* w ./ TFoakey;
        ...
        for n=1:length(variable_names)
            ...
            switch wh_var
                ...
                case "ACC_SPEC"
                    FieldData.(Levelnames{1}).ACC_SPEC(p,:,1)=AP_SPEC1;
                    FieldData.(Levelnames{1}).ACC_SPEC(p,:,2)=AP_SPEC2;
        ...
        for n=1:length(variable_names)
            ...
            switch wh_var
                ...
                case "MAD"
                    FieldData.(Levelnames{2}).MAD(p,:)     = [nan nan];
                case "METHOD"
