...
function result = iter_cell(data, level, addit)
    ...
    if ~isempty(icollect)
     ...
     result=check_cell_matrix(result);% CBluteau addon
  
end

%CBluteau checks if cellarray is actually an array of number
function result=check_cell_matrix(data)
% If data is a cell array of number, then result is an array of numver
result=data;
    if iscell(data)
        dataTmp=cell2mat(data);
        if isnumeric(dataTmp)
            result=dataTmp;
        end 
    end
end
