function [] = L4_epsiflag(filePI,fileTEST,pi,tester)
...
% read in epsi
try
epsiPI = ncread(filePI,'/L4_dissipation/EPSI_FLAGS');
catch
    
epsiTEST = ncread(fileTEST,'/L4_dissipation/EPSI_FLAGS');
...
for ii=1:n
    ...
    box on
    grid on
    ylabel([char(949) ' shear ' int2str(ii) ' (EPSI FLAGS)'])
    if ii==n
