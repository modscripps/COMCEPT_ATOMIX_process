function create_netcdf_fielddata_permute(FieldData,Meta,oname)
...
for ii=1:nGrp
    ...
    write_group(fid,childId(ii),FieldData.(vF{ii}));
...
function write_group(fid,childId,GroupData)
...
dimid=zeros(Ndim,1);
for i=1:Ndim
    wh_dim=GroupData.Meta.dimensions{i};
...
for i=1:Nvar
    ...
    if ~isempty(wh_type)
        ...
        lvl_var_id(i) = netcdf.defVar(childId,wh_var,wh_type,flipud(wh_dims));
    else
